...
class RoomObjects
{
    ...
    /// IterateAction class
    /// </summary>
    /// <param name="roomObjects">list roomObjects</param>
    /// <param name="type">type</param>
    public static void IterateAction(List<Base> roomObjects, Type type)
    {
        var targetobject = roomObjects.Where(e => e.GetType().GetInterfaces().Contains(type)).ToList();

        foreach (var element in targetobject)
        {
            if (type == typeof(IInteractive))
            {
                ((IInteractive)element).Interact();
            }
            else if (type == typeof(IBreakable))
            {
                ((IBreakable)element).Break();
            }
            else if (type == typeof(ICollectable))
            {
                ((ICollectable)element).Collect();
            }
        }
